...
describe("Apps Import", () => {
	it("bilibilib cookie", () => {
		...
		const config = require("data/appImportList.ts");
	...
	it("no random ua", () => {
		...
		const config = require("../lib/config").value;
		expect(config.ua).toBe(
			"RSSHub/1.0 (+http://github.com/DIYgod/RSSHub; like FeedFetcher-Google)"
		);
