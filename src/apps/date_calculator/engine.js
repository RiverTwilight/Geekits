...
const calWhichDay = (dateStart, day) => {
 ...
 console.log(date)
    //先减去这个月剩下的天数
    var restDayOfThisMonth = isLeap(date.year)?leapYearMonth[date.month] - date.day:ordinaryYearMonth[date.month] - date.day
    //day -= restDayOfThisMonth;
...
const calDiffer = (dateEarly, dateLate) => {
	...
	var dateEarly = parseDate(dateEarly)
	var dateLate = parseDate(dateLate)
    ...
    var diffDay = earlyYearDay + lateYearDay
	diffYear.map(year=>{
