...
import { RefreshRounded as RefreshIcon, Close as CloseIcon } from "@mui/icons-material";
...
const RandomChooser = () => {
  ...
  const handleSpin = () => {
    ...
    const newRotation = rotation - extraRotations - (360 / items.length) * newIndex;
  ...
  const handleSavePreset = () => {
    ...
    localStorage.setItem("DECISION_MAKER_PRESETS", JSON.stringify(newPresets));
  };
  ...
  return (
    <StyledBox className={classes.root}>
      ...
      <Box display="flex" justifyContent="center" mb={4} position="relative">
        <ArrowDropDownIcon sx={{ transform: "scale(2, 2)", position: "absolute", top: -20 }} />
        <Box
          sx={{
            borderRadius: "50%",
            padding: 2,
              ...
              return (
                <React.Fragment key={item}>
                  ...
                  <g
                    fontSize="20"
                    className={classes.wheelText}
                    transform={`rotate(${360 * (percent * i + percent / 2)})`}
                    transform-origin="center"
                  >
                    <text x={1.5 * R} y={R} dy={10} textAnchor="middle">
                      {item}
                    </text>
                  </g>
                </React.Fragment>
      ...
      <Box mb={4}>
        ...
        <Box display="flex" justifyContent="space-between">
          ...
          <Button variant="outlined" color="primary" onClick={handleSavePreset}>
            Save Preset
          </Button>
        </Box>
      </Box>
        ...
        <Box>
          ...
          <List>
            {presets.map((preset, index) => (
              <ListItemButton key={index} onClick={() => handleLoadPreset(preset.text)}>
